...
摘抄的用法是**反复进出**：在列表里看到一条，读一眼，回到原处继续。所以：

- **点开一条划线默认不跳转，而是弹出浮层卡片**（原文 + 笔记 + 章节 + 颜色）。
  绝大多数时候你想知道的是「我当时记了什么」，为了回答这个问题把阅读位置丢掉是笔坏账。
  **跳转是卡片里的一个显式按钮**（「跳转到原文」）——想去的时候仍然一步就到。
  导入条目同样可以打开卡片阅读，只是卡片里没有跳转按钮：它们本就锚不住。
- **跳到划线必须记录返回位置**，和从目录、书签跳转一样。
- **有「原位」可回时，顶栏不再自动隐藏**。顶栏本来会在几秒无操作后淡出，而刚跳过去正在读的人
  恰恰就是「几秒无操作」——返回按钮跟着一起消失，跳转就成了单程票。
- **抽屉记住上次停留的页签**。页签状态存在阅读器而不是抽屉里，否则关一次就回到「目录」，
  在整理笔记时每次都要重新点一下。
- **跳过去之后要看得出落在哪一条**。一页上往往不止一条划线，所以目标划线会被短暂加重
  （约 1.4 秒后恢复）——否则「跳转」只意味着页面变了。
- 卡片里的删除与列表里的删除**问同一个问题**（共用确认文案），破坏性操作一律先确认。
